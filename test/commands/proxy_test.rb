...
class CommandsProxyTest < ActiveSupport::TestCase
  ...
  test "run" do
    assert_equal \
      "docker run --name parachute_80 --detach --restart unless-stopped --network kamal --publish 80:80 --volume /var/run/docker.sock:/var/run/docker.sock --volume parachute_80:/root/.config/parachute --log-opt max-size=\"10m\" #{Kamal::Commands::Proxy::DEFAULT_IMAGE}",
  ...
  test "run with logging config" do
    ...
    assert_equal \
      "docker run --name parachute_80 --detach --restart unless-stopped --network kamal --publish 80:80 --volume /var/run/docker.sock:/var/run/docker.sock --volume parachute_80:/root/.config/parachute --log-driver \"local\" --log-opt max-size=\"100m\" --log-opt max-file=\"3\" #{Kamal::Commands::Proxy::DEFAULT_IMAGE}",
      new_command.run.join(" ")
  end
